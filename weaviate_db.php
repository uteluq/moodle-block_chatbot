<?php
/**
 * @copyright 2025 Université TÉLUQ
 */

require_once('../../config.php');

// Enable output buffering at the very beginning
ob_start();

global $CFG, $USER, $COURSE, $DB, $PAGE;

// Get the current user
$currentUserId = $USER->id;
$username = $USER->username;

// To check if the user is a teacher in the current course
$context = context_course::instance($COURSE->id);
$isTeacher = has_capability('moodle/course:update', $context, $currentUserId);

// $log_file = $CFG->dirroot . '/blocks/chatbot/custom_debug.log';

require_login();

error_reporting(E_ALL);
ini_set('display_errors', 1);

require_once($CFG->dirroot . '/blocks/chatbot/classes/weaviate_connector.php');
require_once($CFG->dirroot . '/blocks/chatbot/classes/PDFExtractAPI.php');

// Configuration of API keys and Weaviate URL
$apiUrl = get_config('block_chatbot', 'weaviate_api_url');
$apiKey = get_config('block_chatbot', 'weaviate_api_key');
$cohereApiKey = get_config('block_chatbot', 'cohere_embedding_api_key');

// Initialization of the WeaviateConnector object
$connector = new WeaviateConnector($apiUrl, $apiKey, $cohereApiKey);

try {
    // Check form data
    if (empty($_FILES['files'])) {
        // error_log('Error during file indexing: Missing files.', 3, $log_file);
        throw new Exception("Missing files.");
    }

    $uploadedFiles = $_FILES['files'];

    $userid = $_POST['userid'];
    $courseid = $_POST['courseid'];


    $courseName = 'Collection_course_' . $courseid;
    $connector->deleteCollection($courseName);
    $checkIfCreated = $connector->collectionExists($courseName);
   
    // error_log('Does the collection exist?  ' . $checkIfCreated, 3, $log_file);

    // Create the collection if it does not exist
    if ($checkIfCreated == false && !$connector->createCollection($courseName)) {
        // error_log('Error during collection creation: ' . $connector->getLastError(), 3, $log_file);
        throw new Exception("Error during collection creation: " . $connector->getLastError());
    }

    // Check upload errors
    // foreach ($uploadedFiles['error'] as $key => $error) {
    //     if ($error !== UPLOAD_ERR_OK) {
    //         error_log('Error during file indexing: Upload error.' . $error, 3, $log_file);
    //         throw new Exception("Error during file upload.");
    //     }
    // }

    // Check upload errors
foreach ($uploadedFiles['error'] as $key => $error) {
    if ($error !== UPLOAD_ERR_OK) {
        $message = match($error) {
            UPLOAD_ERR_INI_SIZE => "The file exceeds the maximum size defined in php.ini",
            UPLOAD_ERR_FORM_SIZE => "The file exceeds the maximum size specified in the HTML form",
            UPLOAD_ERR_PARTIAL => "The file was only partially uploaded",
            UPLOAD_ERR_NO_FILE => "No file was uploaded",
            UPLOAD_ERR_NO_TMP_DIR => "Missing temporary folder",
            UPLOAD_ERR_CANT_WRITE => "Failed to write file to disk",
            UPLOAD_ERR_EXTENSION => "A PHP extension stopped the file upload",
            default => "Unknown upload error"
        };
        
        // error_log("Error during file indexing: $message (code: $error)", 3, $log_file);
        throw new Exception("Upload error: $message");
    }
}

    // Use the global $CFG->dataroot variable to build the storage directory path
    $uploadDir = $CFG->dataroot . '/filedir/';

    foreach ($uploadedFiles['tmp_name'] as $key => $fileTmpName) {
        $fileName = $uploadedFiles['name'][$key];
        $newFileName = uniqid('file_', true) . '-' . $fileName;
        $destination = $uploadDir . $newFileName;

        // Move the file to the destination directory
        move_uploaded_file($fileTmpName, $destination);

        $adobe_pdf_client_id = get_config('block_chatbot', 'adobe_pdf_client_id');
        $adobe_pdf_client_secret = get_config('block_chatbot', 'adobe_pdf_client_secret');

        // Extract the file content
        $pdfExtractor = new PDFExtractAPI($adobe_pdf_client_id, $adobe_pdf_client_secret);
        $extractedText = $pdfExtractor->processPDF($destination);


        // Generate a unique name for the text file
        $newFileTxtName = uniqid('file_', true) . '-' . $fileName . '.txt';
        $destinationTxt = $uploadDir . $newFileTxtName;


        $in = $connector->indexTextFile($destinationTxt, $courseName, $courseName);

        if (!$in) {
            throw new Exception("Error during file indexing: " . $connector->getLastError());
        }
    }

    // Successful response
    $response = ['success' => true, 'message' => "All files have been successfully indexed."];
    $message = $response['message'];

    if (!mb_detect_encoding($message, 'UTF-8', true)) {
        $response['message'] = utf8_encode($message); // Force encoding to UTF-8.
    }
} catch (Exception $e) {
    // Error handling
    $response = ['error' => $e->getMessage()];
    // error_log('Exception thrown: ' . $e->getMessage(), 3, $log_file);
}

// Clear all output buffers
while (ob_get_level()) {
    ob_end_clean();
}

// Set the header for JSON content type
header('Content-Type: application/json');

// Logging

// Output the JSON
echo json_encode($response, JSON_UNESCAPED_UNICODE);

exit;